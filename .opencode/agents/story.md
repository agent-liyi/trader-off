...
- **EARS 结构化表述**：功能需求是否可用 EARS 句式完整覆盖预期行为？**若已满足 EARS 要求，则不向用户追问**；仅当缺失、含糊或自相矛盾时才向用户澄清。
...
**退出条件**：
- [ ] Agent 已自检满足 4W + EARS 完整性（或已就缺失项向 Human 澄清）。
...
*—— 本故事由 M-STORY Agent 于 {日期} 生成；经 Human 确认后：Go → 进入 M-FOUND，Park / No-Go → 存档入 Backlog 并标记（story 永久保留，不删除）。*
```
